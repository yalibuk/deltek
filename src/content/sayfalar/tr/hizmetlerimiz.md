---
baslik: "Hizmetlerimiz"
seoBaslik: "Hizmetlerimiz: HDD, Boru Sürme, Kazısız Boru Yenileme"
menuSira: 3
menuAd: "Hizmetlerimiz"
ozet: "Yönlendirilebilir yatay sondaj (HDD), boru sürme/çakma, kazısız boru yenileme ve akıllı altyapı danışmanlığı; kablo, basınçlı hat ve nehir geçişleri."
ilgili:
  - "yonlendirilebilir-yatay-sondaj"
  - "boru-surmecakma"
  - "boru-yenileme"
  - "akilli-altyapi"
  - "referanslar"
anahtarKelimeler: "yatay sondaj hizmeti, HDD hizmeti, boru sürme, auger boring, kazısız boru yenileme, pipe bursting, akıllı altyapı danışmanlığı, nehir geçişi, kablo geçişi"
duzen: hizmet

hizmetler:
  - baslik: "Yönlendirilebilir Yatay Sondaj"
    kisaltma: "HDD"
    ikon: sondaj
    href: "/yatay-sondaj/"
    metin: "Yer altında üç boyutlu yön verilebilen mekanizması sayesinde artık altyapı projelerinizi daha kolay ve zahmetsiz döşüyoruz. Varolan hatlara zarar vermeden, yeryüzü aktivitelerini ve trafiği aksatmadan kablo ve boru hatlarınızı yer altına alıyoruz. Daha az kazı, daha çok teknoloji. Giriş/Çıkış noktasını verin, gerisini düşünmeyin!"
  - baslik: "Boru Sürme, Boru Çakma / İtme"
    kisaltma: "Auger Boring"
    ikon: boru
    href: "/boru-surmecakma/"
    metin: "2000 mm çapa kadar çelik borularınızı insan hayatını tehlikeye atmadan, kısa zamanda ve adil maliyetlerle yer altına alıyoruz. Ayarlanabilir şaft boyutları ve yüksek makine kapasitesi sayesinde uzun mesafelerde bile ara istasyona ihtiyaç duymadan kesin ve etkin çözüm üretiyoruz."
  - baslik: "Kazısız Boru Yenileme"
    kisaltma: "Pipe Bursting"
    ikon: yenileme
    href: "/boru-yenileme/"
    # Hizmetlerimiz sayfasındaki metin "Ayrıca bütün bu işlemleri sadece" diye
    # yarım kalıyordu ve arşivde de hiç tamamlanmamıştı; yerine canlı ana
    # sayfadaki aynı hizmet kartının eksiksiz metni kullanıldı.
    metin: "Her gün artan altyapı ihtiyaçları, şehir merkezlerinde çözülmesi en zor sorunlardan biri olarak karşımıza çıkıyor. Yoğun günlük şehir aktiviteleri yolları ve alanları uzun süre kapatmaya izin vermeyebiliyor. Teknolojik boru yenileme/patlatma sistemlerimizle kazı yapmadan, eski hattın yerine — üstelik isterseniz daha büyük çaplarda — yeni boruları döşüyoruz!"
  - baslik: "Akıllı Altyapı Danışmanlığı"
    kisaltma: "Smart Undergrounding"
    ikon: akilli
    href: "/akilli-altyapi/"
    metin: "Güncel altyapı hattı sorunlarınıza yarının penceresinden bakarak etkin çözümler üretiyoruz. Kazısız altyapı sektöründe hem yurt içinde hem de yurt dışında elde ettiğimiz haklı gurur ve tecrübeyi projelerinize hayat vermek için hizmetinize sunuyoruz. Bir de bizim gözümüzden bakın!"

neden:
  baslik: "Neden Bizi Seçtiler?"
  metin: "Şimdiye kadar birlikte çalıştığımız onlarca firmanın her birinin öncelikli tercihi olmaktan gurur duyuyoruz. Diğer taraftan bu başarının arkasına saklanmayıp çıtayı her seferinde daha da yukarı çıkarmayı hedefliyoruz. Ticari hayatta dostlukların son derece zor muhafaza edildiğinin bilincindeyiz. Bu anlamda iş ortaklarımızın bizi seçmelerinde etkili olan başlıca kriterleri şu şekilde sıralayabiliriz:"
  maddeler:
    - "Fırsat maliyeti gözetmiyoruz. Her ortamda adil fiyat politikası güdüyoruz."
    - "Çalışkanız. İşimiz için fazladan mesai harcamaktan çekinmiyoruz."
    - "Kendimizi ve makine parkımızı sürekli yeniliyoruz."
    - "Zamanında iş teslim ediyor; iş ortaklarımıza zaman avantajı sunuyoruz."
    - "İş-Ekip-Makine optimizasyonu yapıyor, beklenmedik maliyetler yaratmıyoruz."
  gorsel: "/images/icerik/deltek-yatay-sondaj-yer-belirleme.webp"
  gorselAlt: "Deltek yatay sondaj yer belirleme ve yönlendirme cihazları — HDD ekipman parkı"

projeler:
  baslik: "Son Projeler"
  metin: "Bitirdiğimiz son projelerden kesitleri buradan takip edebilir; kullandığımız ekipman ve yüksek teknolojinin iş ortaklarımıza nasıl katma değer yarattığı hakkında fikir edinebilirsiniz. Hedefimiz: SIFIR HATA, %100 MEMNUNİYET!"
  galeri:
    - { foto: "/images/icerik/deltek-kazisiz-gecis-sahasinda-yatay-sondaj.webp", alt: "Deltek kazısız geçiş sahasında yatay sondaj makinesi ile çalışma" }
    - { foto: "/images/icerik/auger-boring-boru-surme-semasi.webp", alt: "Auger boring (boru sürme) şeması — helezonlu delgi kafası ile çelik borunun yer altına sürülmesi" }
    - { foto: "/images/icerik/yonlendirilebilir-yatay-sondaj-sahasi-hdd-makinesi.jpg", alt: "Yönlendirilebilir yatay sondaj sahası — HDD makinesi ve delgi tijleri" }

alanlar:
  baslik: "Uygulama Alanlarımız"
  ogeler:
    - baslik: "Kablo Geçişleri"
      ikon: kablo
      href: "/iletisim/"
      metin: "Elektrik hatları, alçak/orta/yüksek gerilim enerji hatları, telekomünikasyon kabloları (fiber optik, koaksiyel vb.), görüntü ve data hattı kabloları, sinyalizasyon kabloları."
    - baslik: "Basınçlı Hat Geçişleri"
      ikon: basinc
      href: "/iletisim/"
      metin: "Petrol ve doğalgaz boru hatları, içme suyu hatları, endüstriyel kimyasal iletim hatları, jeotermal iletim hatları, basınçlı kanalizasyon hatları, akaryakıt hatları, yangın hidrant hatları."
    - baslik: "Cazibeli Hat Geçişleri"
      ikon: cazibe
      href: "/iletisim/"
      metin: "Kanalizasyon hatları, yağmur suyu hatları, su kanalı hatları, cazibeli akan doğal su hatları, doğal eğimli drenaj ve deşarj hatları, kollektör hatları, tarımsal sulama hatları."
    - baslik: "Nehir Geçişleri"
      ikon: nehir
      href: "/iletisim/"
      metin: "Nehir-akarsu-dere geçişleri, karayolu ve otoyol geçişleri, köprü ve viyadük altı geçişler, endüstriyel tesis altı geçişler, kazı izni verilmeyen tarihi eser geçişleri, deniz ve boğaz geçişleri, kavşak geçişleri."
---

Deltek, 20 yılı aşkın saha tecrübesiyle İstanbul ve İzmir ofislerinden Türkiye
genelinde **yatay sondaj**, **boru sürme/çakma** ve **kazısız boru yenileme**
hizmeti veren bir kazısız geçiş yüklenicisidir. Yönlendirilebilir yatay sondajda
660 tona, boru sürmede 1.800 tona varan makine parkımızla 2 mm'den 2000 mm'ye
kadar her çapta boru ve kablo hattını nehir, otoyol, demiryolu ve şehir içi
geçişlerde açık kazı yapmadan yer altına alıyoruz. Aşağıdaki dört ana hizmet
için ücretsiz fizibilite ve keşif çalışması yapıyoruz.
